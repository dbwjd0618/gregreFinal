	...
	<div class="main-content">
		...
		<div class="row text-center">
			<div class="col-md-12">
					...
					<table class="table table-bordered">
						...
						<tr>
							...
							<td class="text-left"
								...
								data-date="2020-03-25" data-date-format="yyyy-mm-dd" /> ~ <input
								type="text" name="endDate" class="form-control"
								style="width: 100px; display: inline-block;" id="edate" value=""
								data-date="2020-04-01" data-date-format="yyyy-mm-dd" />
							...
							<td class="text-center" rowspan="5"
								style="background-color: #fff; vertical-align: middle;"><button
									class="btn btn-primary btn-lg" id="searchBtn" type="button"
									onclick="searchPram();">검 색</button></td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td class="text-left"
								style="background-color: #fff; vertical-align: middle;"><select
								name="payMethod" class="form-control" id="pay"
								style="width: 200px;">
							...
							<td class="text-left" style="background-color: #fff;" colspan="3">
								<label> <input type="checkbox" name="ostatus" value="A">
									<span
									style="font-size: 12px; color: #5f6164; vertical-align: top; line-height: 21px; margin-right: 10px;">입금대기</span>
							</label> <label> <input type="checkbox" name="ostatus" value="B">
									<span
									style="font-size: 12px; color: #5f6164; vertical-align: top; line-height: 21px; margin-right: 10px;">결제완료</span>
							</label> <label> <input type="checkbox" name="ostatus" value="C">
									<span
									style="font-size: 12px; color: #5f6164; vertical-align: top; line-height: 21px; margin-right: 10px;">배송준비</span>
							</label> <label> <input type="checkbox" name="ostatus" value="D">
									<span
									style="font-size: 12px; color: #5f6164; vertical-align: top; line-height: 21px; margin-right: 10px;">배송중</span>
							</label> <label> <input type="checkbox" name="ostatus" value="E">
									<span
									style="font-size: 12px; color: #5f6164; vertical-align: top; line-height: 21px; margin-right: 10px;">배송완료</span>
							</label>
							</td>
						</tr>
				...
				<form method="post"
					action="${PageContext.request.contextPath}/shop/admin/order/search.do"
					role="form" class="form-horizontal" name="regi_form" id="regi_form">

					<div class="col-md-12"
						style="text-align: left; margin-bottom: 8px; padding-left: 0px;">
						<span class="col-md-12 text-left b"
							style="font-size: 1.3em; padding-left: 0px;">총 주문건수 : <strong
							class="text-danger">${totalOrders }</strong>개
							<button class="btn btn-primary btn-sm" type="button" id="ChkSms"
								style="margin-left: 20px;">선택 SMS발송</button>
					...
					<table class="table table-bordered">
						...
						<tr>
							...
							<th class="text-center active">주문자명<br>주문자전화
							</th>
							<th class="text-center active">수령인명<br>수령인핸드폰
							</th>
							<th class="text-center active">우편번호<br>주소1<br>주소2
							</th>
							<th style="width: 250px;" class="text-center active">상품정보<br>(상세)
							</th>
							<th class="text-center active">상품금액</th>
							<th class="text-center active">배송비<br>총결제금액
							</th>
							<th class="text-center active">택배사<br>배송번호
							</th>
							<th class="text-center active">결제방법</th>
							<th class="text-center active">결제상태<br>입금자명
							</th>
						...
						<c:forEach items="${list}" var="l">
							<tr style="background-color: #ffffff;">
								<td class="text-center vertical-middle"><input
									type="checkbox" name="chkDel" value="TLYChVDHcld_73929">
									<input type="hidden" name="phone[]" value="${recpt.phone }">
								</td>
								<td class="text-center vertical-middle">${l.orderNo}</td>

								<td class="text-center vertical-middle">${l.orderDate }</td>
								<td class="text-center vertical-middle">${l.memberName }<br>
									<a class="btn btn-warning btn-sm sendSMSForm" recvphone="">${l.phone}</a>
								</td>
								<td class="text-center vertical-middle">${l.recptName }<br>
									<a class="btn btn_black btn-sm sendSMSForm" recvphone="1234">${l.recptPhone }</a>
								</td>

								<td class="text-left vertical-middle">${l.recptZipcode }<br>
									${l.recptAddr }<br> ${l.recptDetailAddr }
								</td>
								<td class="text-left vertical-middle"><img
									src="${pageContext.request.contextPath}/resources/upload/shop/productMainImg/${l.attachList.get(0).renamedImg}"
									style="width: 200px;" alt=""><br> 
									
									<strong><a href="${pageContext.request.contextPath }/shop/product/detail.do?productId=${l.productId }">${l.attachList.get(0).productName}</a></strong><br>
								<c:if test="${l.optionName ne null}">		
								<br><p> opt : ${l.optionName }/${l.optionValue }(${l.prodCount }개)</p>
								</c:if>
								</td>
								<td class="text-right vertical-middle">${l.totalPrice }</td>
								<td class="text-right vertical-middle">${l.totalDeliveryFee }
									원<br>${l.totalPrice+l.totalDeliveryFee } 원
								</td>
								<td class="text-right vertical-middle">우체국택배<br>${l.deliveryNo}</td>
								<td class="text-right vertical-middle">${l.payMethod }</td>
								<td class="text-center vertical-middle"><span
									class="dev_stat_red">${l.payState }</span></td>
								<td class="text-center vertical-middle"><span
									class="dev_stat_green">${l.deliveryState }</span></td>
								<td class="text-center vertical-middle">
									<button class="btn btn-detail btn-norad"
										...
										type="button">제품보기</button>
									<button class="btn btn-warning btn-norad"
										onclick="deleteOrderList('${l.orderNo}');" type="button">주문취소</button>
								</td>
							</tr>
						</c:forEach>
...
<!--메모 모달창 -->
<div class="modal fade" id="sendMemoForm" tabindex="-1" role="dialog"
	aria-labelledby="sendMemoForm" aria-hidden="true" alreadyLoad="0">
		...
		<div class="modal-content" id="sendMemoFormContent">
			<div class="sms_wrap" style="margin-top: 0px;">
				<form id="smsForm">
					...
					<p>
						<textarea class="form-control" rows="5" name="memoComm"
							id="memoComm" style=""></textarea>
					</p>


					<div class="text-center">
						<a href="javascript:;" class="btn btn-info btn-sm"
							id="memoSendBtn">수정하기</a> <a href="javascript:;" id="closebtn"
							class="btn btn-default btn-sm" data-dismiss="modal">닫기</a>
					</div>
				</form>
			</div>
		</div>
...
<!--메모 모달창끝 -->
<!--sms 개별모달창 -->
<div class="modal fade" id="sendSMSForm" tabindex="-1" role="dialog"
	aria-labelledby="sendSMSForm" aria-hidden="true" alreadyLoad="0">
	<div class="modal-dialog">
		<div class="modal-content" id="sendSMSFormContent">
			<div class="sms_wrap" style="margin-top: 0px;">
				<form id="smsForm">
					...
					<p>
						<textarea class="form-control" rows="3" name="smsMessage"
							id="smsMessage"></textarea>
					</p>
					<div class="sms_number">
						<ul>
							<li class="clearfix"><span class="name">받는 사람</span> <span
								class="phone"><input type="text" name="smsReceive"
									id="smsReceive" class="form-control input-sm" value=""></span>
							</li>
							<li class="clearfix"><span class="name">보내는사람</span> <span
								class="phone"><input type="text" name="smsSender"
									id="smsSender" class="form-control input-sm"
									value="01089671359" readonly></span></li>
						</ul>
					</div>
					<div class="text-center">
...
<script type="text/javascript">
	...
	function deleteOrderList(val) {

		var orderNo = val;
		console.log(orderNo)
		var con = confirm("삭제하시겠습니까? \n 삭제하시면 복구 할 수 없습니다.");
		if (con) {
			location.href = "${pageContext.request.contextPath }/shop/admin/order/delete.do?orderNo="
					+ orderNo;
		} else {
			return false;
		}
			...
			for (i = 0; i < nChk.length; i++) {
				if (nChk[i].checked) { //체크되어 있을경우 
					if (form.ostatus.value == '') {
						...
						form.ostatus.value = form.ostatus.value + ','
								+ nChk[i].value; //구분자로 +=
					}
				}
			}
	...
	function getDateStr(myDate) {
		var mm = String(myDate.getMonth() + 1).length === 1 ? '0'
				+ String(myDate.getMonth() + 1) : String(myDate.getMonth() + 1);
		var dd = String(myDate.getDate()).length === 1 ? '0'
				+ String(myDate.getDate()) : String(myDate.getDate());
	...
	$(document)
			.ready(
					function() {
						...
						//현재 년월일 뽑아내는것.
						var today_val = new Date().toISOString().substring(0,
								10);
						...
						$("body").on(
								"copy",
								".zclip",
								function(/* ClipboardEvent */e) {
									e.clipboardData.clearData();
									e.clipboardData.setData("text/plain", $(
											this).data("zclip-text"));
									e.preventDefault();
									alert("URL이 복사되었습니다.\n Ctrl+V로 붙여넣기 하세요");
								});
						...
						$('.modifyMemo')
								.click(
										function() {
											var orderNum = $(this).attr(
													'orderNum'), orderNumMemo = $(
													this).attr('orderNumMemo');
						...
						$('#memoSendBtn')
								.click(
										function() {
											var adminNum = $('#momoPop').val(), adminMemo = $(
													'#memoComm').val();
