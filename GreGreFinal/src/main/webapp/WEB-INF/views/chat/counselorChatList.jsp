...
<jsp:include page="/WEB-INF/views/chat/chatHeader.jsp"/>
	<table class="table">
		...
		<tbody>
			<c:forEach items="${recentList }" var="m" varStatus="vs">
				<tr chatNo='<c:out value="${m.CHAT_ID}.${m.MEMBER_ID}"/>' />
				<%-- el의 문자열 더하기 연산대신 jstl out태그 사용 --%>
	...
	<script>
		function goChat(chatId) {
			open("${pageContext.request.contextPath}/counselor/chat/" + chatId,
					chatId, "width=500, height=500", false);
		}
		...
		function subscribeByPage(){
			...
			let intervalId = setInterval(()=>{
				...
				if(conntionDone==false && stompClient.connected){
					...
					// subscribe message
			        stompClient.subscribe('/chat/counselor/push', function(message) {
			            console.log("receive from /chat/counselor/push :", message);
